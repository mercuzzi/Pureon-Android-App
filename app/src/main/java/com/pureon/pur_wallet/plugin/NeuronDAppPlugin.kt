...
import com.pureon.pur_wallet.activity.AppWebActivity.RESULT_CODE_SCAN_QRCODE
import com.pureon.pur_wallet.activity.QrCodeActivity
import com.pureon.pur_wallet.constant.CytonDAppCallback
import com.pureon.pur_wallet.item.dapp.DeviceMotion
import com.pureon.pur_wallet.item.dapp.Gyroscope
import com.pureon.pur_wallet.item.dapp.BaseCytonDAppCallback
import com.pureon.pur_wallet.util.JSLoadUtils
import com.pureon.pur_wallet.util.SensorUtils
...
class CytonDAppPlugin(private val mContext: Activity, private val mWebView: WebView) {
    ...
    fun checkPermissions(info: String, callback: String) {
        ...
        var permissionItem: com.pureon.pur_wallet.item.dapp.Permission
        if (permissionList !== null) {
            permissionItem = com.pureon.pur_wallet.item.dapp.Permission(AndPermission.hasPermissions(mContext, permissionList))
            mWebView.post { JSLoadUtils.loadFunc(mWebView, callback, Gson().toJson(permissionItem)) }
        } else {
            permissionItem = com.pureon.pur_wallet.item.dapp.Permission(0, CytonDAppCallback.NO_PERMISSION_CODE, CytonDAppCallback.NO_PERMISSION, false)
            mWebView.post { JSLoadUtils.loadFunc(mWebView, callback, Gson().toJson(permissionItem)) }
        }
    }
    ...
    fun requestPermissions(info: String, callback: String) {
        ...
        if (permissionList !== null) {
            AndPermission.with(mContext)
                    ...
                    .rationale(RuntimeRationale())
                    .onGranted {
                        permissionItem = com.pureon.pur_wallet.item.dapp.Permission(true)
                        mWebView.post { JSLoadUtils.loadFunc(mWebView, callback, Gson().toJson(permissionItem)) }
                    }
                    .onDenied { permissions ->
                        PermissionUtil.showSettingDialog(mContext, permissions)
                        permissionItem = com.pureon.pur_wallet.item.dapp.Permission(false)
                        mWebView.post { JSLoadUtils.loadFunc(mWebView, callback, Gson().toJson(permissionItem)) }
                    }
                    .start()
        } else {
